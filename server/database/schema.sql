CREATE TABLE user(
   ...
   is_pegi_ok BOOLEAN NOT NULL,
   role BOOLEAN NOT NULL,
   avatar VARCHAR(255)  NOT NULL,
   CONSTRAINT PK_user PRIMARY KEY(ID),
   CONSTRAINT AK_user UNIQUE(login)
);
...
CREATE TABLE like_(
   ...
   CONSTRAINT FK_like__user FOREIGN KEY(ID_user) REFERENCES user(ID),
...
CREATE TABLE track(
   ...
   CONSTRAINT FK_track_user FOREIGN KEY(ID_user) REFERENCES user(ID),
...
CREATE TABLE media_user(
   ...
   CONSTRAINT FK_media_useruser FOREIGN KEY(ID_user) REFERENCES user(ID),
   CONSTRAINT FK_media_usermedia FOREIGN KEY(ID_media) REFERENCES media(ID)
);
...
CREATE TABLE episode_user(
   ...
   CONSTRAINT FK_episode_useruser FOREIGN KEY(ID_user) REFERENCES user(ID),
   CONSTRAINT FK_episode_userepisode FOREIGN KEY(ID_episode) REFERENCES episode(ID)
);
